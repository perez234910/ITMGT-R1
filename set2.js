...
function caesarCipher(message, shift) {
    ...
    for (let char of message) {
        if (char === ' ') {
            ...
            const newCode = ((code - 65 + normShift) % 26) + 65;
...
function shiftByLetter(letter, letterShift) {
    ...
    const shiftCode = letterShift.charCodeAt(0);
    const shiftAmount = shiftCode - 65;
    const newLetterCode = ((letterCode - 65 + shiftAmount) % 26) + 65;
...
function vigenereCipher(message, key) {
    ...
    let keyIndex = 0 ;
     ...
     for (let char of message) {
        ...
        const shiftAmount = keyCode - 65;
        const newCharCode = ((messageCode - 65 + shiftAmount) % 26) + 65;
...
function scytaleCipher(message, shift) {
    ...
    for (let i = 0; i < message.length; i++) {
        ...
        const col = i % shift;
        const charIndex = row + (numRows * col); 
...
function scytaleDecipher(message, shift) {
    ...
    for (let i = 0; i < message.length; i++) {
        ...
        const col = i % numRows; 
        const charIndex = col * shift + row; 

        decodedMessage += message[charIndex];
    }
